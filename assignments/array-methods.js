...
//for their running bibs. Combine both the first and last names and populate a 
//new array called `fullNames`. This array will contain just strings.
let fullNames = [];
...
// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in 
//uppercase because the director BECAME DRUNK WITH POWER. Populate 
...
// Problem 1
// The director wants to know all the companies names.
let comNames = [];
runners.forEach(function (item) {
  return comNames.push(item.company_name);
});
console.log(comNames);

// Problem 2
// The director was not satisified so to demonstrate his power he also
//wants to know all the names associated with these companies .

let runnercomNames = [];
runners.forEach(function (item) {
  return runnercomNames.push(item.company_name, item.first_name, item.last_name);
});
console.log(runnercomNames.sort());
